...
static bool playerDraw(Card dealer, const Hand &player) {
    ...
    if (value.soft) {
        return (value.count <= 17) || (value.count == 18 && !(dealer.spot == TWO || dealer.spot == SEVEN || dealer.spot == EIGHT));
        /*if (value.count <= 17) {
            return true;
        ...
        return (value.count <= 11) || (value.count == 12 && !(dealer.spot >= FOUR && dealer.spot <= SIX)) || (value.count >= 13 && value.count <= 16 && !(dealer.spot >= TWO && dealer.spot <= SIX));
        /*if (value.count <= 11) {
            return true;
